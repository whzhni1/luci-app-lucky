...
# 创建目录结构
WORK=$(mktemp -d)
mkdir -p "$WORK"/{control,data/usr/bin,data/etc/init.d,data/etc/config} output

# 复制文件
cp "$BINARY" "$WORK/data/usr/bin/lucky" && chmod 755 "$WORK/data/usr/bin/lucky"
[ -f "$FILES/lucky.init" ] && cp "$FILES/lucky.init" "$WORK/data/etc/init.d/lucky" && chmod 755 "$WORK/data/etc/init.d/lucky"
[ -f "$FILES/lucky.config" ] && cp "$FILES/lucky.config" "$WORK/data/etc/config/lucky"
[ -f "$FILES/luckyarch.bin" ] && cp "$FILES/luckyarch.bin" "$WORK/data/usr/bin/luckyarch" && chmod 755 "$WORK/data/usr/bin/luckyarch"
...
#!/bin/sh
/etc/init.d/lucky stop 2>/dev/null; /etc/init.d/lucky disable 2>/dev/null; exit 0
EOF
...
cp *.ipk "$OLDPWD/output/"
rm -rf "$WORK"

echo "✅ output/lucky_${VERSION}-1_${OPENWRT_ARCH}.ipk"
